...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        # 5 попыток соединения, если успехх - флаг в True
        connected = False
        for i in range(5):
        ...
        # Если соединитьься не удалось - исключение
        if not connected:
        ...
        # Посылаем серверу приветственное соббщение и получаем ответ
        try:
            with socket_lock:
        ...
        # Если все хорошо, сообщение об установке соединения.
    ...
    def process_server_ans(self, message):
        ...
        # Если это сообщение от пользователя, добавляем в базу, даем сигнал о новом сообщении
    ...
    def remove_contact(self, contact):
        logger.debug(f'Удаление контакт {contact}')
        req = {
        ...
        while self.running:
            ...
            with socket_lock:
                ...
                # проблемы с соединением
                    ...
                    logger.debug(f'Потеряно осединение с сервером.')
                    self.running = False
                    self.connection_lost.emit()
