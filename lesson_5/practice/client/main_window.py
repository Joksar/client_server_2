...
class ClientMainWindow(QMainWindow):
    def __init__(self, database, transport):
        ...
        self.current_chat = None # Текущий контакт, с которым идет обмен сообщениями
    ...
    def set_disabled_input(self):
        # Надпись - получатель
        self.ui.label_new_message.setText('Для выбора получателя дважды щелкните на нем в окне контактов.')
    ...
    def history_list_update(self):
        ...
        self.history_model.clear()
        # Берем не более 20 последних записей
        length = len(list_messages)
        start_index = 0
        if length > 20:
            start_index = length - 20

        # Заполнение модели записями, стоит разделить входящие и исходящие выравнимванием и сделать рахным фоном
        for i in range(start_index, length):
            item = list_messages[i]
            if item[1] == 'in':
                mess = QStandardItem(f'Входящее от {item[3].replace(microseconds=0)}:\n {item[2]}')
    ...
    def set_active_user(self):
        ...
        # Заполняем окно историю сообщение по требуемому пользователю
    ...
    # Функция-обработчик добавления, сообщает серверу, обновляет таблицу и спислк контактов
    ...
    def delete_contact(self, item):
        ...
        try:
            ...
            item.close()
            # Если удален активный пользователь, то деактивируем поле ввода.
    ...
    def send_message(self):
        # Текст в поле, проверяем, что поле не пустоеЭ затем забирается сообщение и поле очищается
        ...
        try:
            ...
            self.messages.critical(self, 'Ошибка', 'Потеярно соединение с сервером!')
            self.close()
        else:
    ...
    def message(self, sender):
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                ...
                print('NO')
                # Раз нет, спрашиваем, хотим ли добавть пользователя в контакты.
                if self.messages.question(self, 'Новое сообщение',
                                          f'Получено новое сообщение от {sender}.\n'
